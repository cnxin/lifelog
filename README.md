...
```
https://github.com/cnxin/lifelog/raw/main/lifelog-v0.1.0-test.56.apk
```
...
- 修复人物喜好 / 禁忌清空后再次编辑恢复默认值的问题，喜好项目改为可删除的标签式编辑。
- 标签输入统一支持顿号、逗号、分号和换行分隔，人物、地点、回忆表单提示保持一致。
- 地点平台链接改为多行编辑，避免编辑时丢失美团、点评、抖音或自定义链接。
- 地点分享解析继续增强，覆盖高德 Android scheme、美团外卖 scheme、点评短链和换行字段。
- 编辑已有地点或回忆时，清空省份、城市、商场、心情等字段会保持为空，不再自动回填默认值。
